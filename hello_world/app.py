    ...
    table = dynamodb.Table("YourDynamoDBTableName")  # Replace with your actual table name
    ...
    def handle_send_message(event):
        try:
            ...
            instruction = (
                ...
                f"If the user just said an abstract time like 'today' or 'yesterday', you can guess it with the current date, "
    ...
    def extract_location(full_context):
        prompt = f"Create the location in JSON format like {{\"lat\": 35.6895, \"lon\": 139.6917}} from this message: \"{full_context}\". If not available, return 'unknown'."
        response = bedrock.invoke_model(
            modelId="anthropic.claude-3-5-sonnet-20240620-v1:0",
            contentType="application/json",
            accept="application/json",
        ...
        reply = json.loads(response["body"].read())["content"][0]["text"].strip()
        print("Location Response:", reply)
        try:
            location = json.loads(reply)
            if isinstance(location, dict) and "lat" in location and "lon" in location:
                return location
        except Exception as e:
            logger.warning(f"⚠️ Failed to parse location: {e}")
