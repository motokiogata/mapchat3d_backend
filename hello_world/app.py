...
def handle_trigger_analytics(connection_id: str, action: Dict[str, Any], apig, event) -> Dict[str, int]:
    ...
    try:
        ...
        # ✅ RESTORED: Check if we already have basic info from parallel gathering
        if (orchestrator and 
            orchestrator.state.questions_asked.get("datetime") and 
            orchestrator.state.questions_asked.get("description")):
            
            # We have basic info, move DIRECTLY to analytics processing
            logger.info(f"✅ Basic info already collected, moving to analytics processing")
            orchestrator.state.phase = ConversationPhase.ANALYTICS_PROCESSING
            save_orchestrator_state(connection_id, orchestrator)
            
            # Trigger detailed analytics immediately
            return handle_start_detailed_analytics(
                connection_id, 
                {"action": "start_detailed_analytics", "progress": "Running detailed analysis (Step 4/6)"}, 
                apig, 
                event
            )
        else:
            # Need to collect basic info first
            logger.info(f"⚠️ Basic info not complete, moving to basic info gathering")
            ...
            progress = action.get("progress", "Gathering information (Step 3/6)")
...
def build_comprehensive_accident_context(connection_id: str, orchestrator: TaskOrchestrator) -> str:
    ...
    if orchestrator and orchestrator.state.collected_data:
        collected = orchestrator.state.collected_data
        
        if collected.get("datetime"):
            context_parts.append(f"ACCIDENT TIME: {collected['datetime']}")
        
        if collected.get("description"):
            context_parts.append(f"ACCIDENT DESCRIPTION: {collected['description']}")
    
    # 2. Add analytics results (detailed investigation)
    if orchestrator and orchestrator.state.analytics_result:
        analytics = orchestrator.state.analytics_result
        
        if analytics.get("final_analysis"):
            context_parts.append(f"DETAILED ANALYSIS: {analytics['final_analysis']}")
        ...
        if analytics.get("route_data"):
            ...
            if vehicles.get("user_vehicle"):
                ...
                context_parts.append(f"USER VEHICLE: Coming from {user_vehicle.get('path', {}).get('origin', {}).get('direction', 'unknown')} direction, intended to {user_vehicle.get('path', {}).get('intended_destination', {}).get('maneuver', 'unknown')}")
            ...
            if traffic.get("signals"):
                ...
                context_parts.append(f"TRAFFIC SIGNALS: User had {signals.get('user_vehicle_signal', 'unknown')}, Other had {signals.get('other_vehicle_signal', 'unknown')}")
    
    # 3. Add modifier answers if available
    if orchestrator and orchestrator.state.modifier_answers:
        context_parts.append("MODIFIER ANSWERS:")
...
def load_csv_from_s3(csv_filename):
    ...
    try:
        logger.info(f"📁 Loading CSV: {csv_filename} from bucket: {CSV_BUCKET}")
...
def find_matching_pattern(category, full_context):
    ...
    for file in csv_files:
        try:
            ...
            if df.empty:
                logger.warning(f"⚠️ Empty or failed to load: {file}")
                continue
                
            logger.info(f"📊 Processing {len(df)} cases from {file}")
    ...
    if not all_cases:
        logger.error(f"❗ No valid cases loaded for category: {category}")
        return None
    ...
    prompt = (
        ...
        f"- case_number: The case number from the list\n"
        f"- confidence_score: A number from 0-10 indicating similarity\n"
        f"- reasoning_details: Brief explanation of why this case is similar\n"
        f"- fault_ratio: The basic fault ratio from the case\n"
        f"- applicable_modifications: List of key modifiers that might apply\n\n"
        f"Output ONLY a JSON array of 5 objects, sorted by descending confidence_score.\n"
        f"Do not include any markdown formatting or explanations."
    )
    ...
    try:
        ...
        try:
            ...
            logger.error(f"❗ Failed to parse similarity JSON: {e}")
            logger.error(f"❗ Raw response: {result_text}")
            return None
            
    except Exception as e:
        logger.error(f"❗ Error calling Bedrock for similarity analysis: {e}")
        return None
...
def store_to_dynamodb(connection_id, datetime_str, location, similar_cases=None):
    """Store to DynamoDB with top 3 similar cases - IMPROVED ERROR HANDLING"""
    try:
        logger.info(f"🔄 Starting DynamoDB storage for {connection_id}")
        logger.info(f"📅 DateTime: {datetime_str}")
        logger.info(f"📍 Location: {location}")
        logger.info(f"📊 Similar cases count: {len(similar_cases) if similar_cases else 0}")
        ...
        if "lat" not in location or "lon" not in location:
            raise ValueError(f"Location missing lat/lon: {location}")

        # Get normalized datetime
        final_datetime = datetime_str
        ...
        # Process similar cases with better error handling
        similar_data = None
        if similar_cases and len(similar_cases) > 0:
            logger.info(f"🔍 Processing {len(similar_cases)} similar cases")
            similar_data = []
            
            for i, case in enumerate(similar_cases[:3]):
                try:
                    logger.info(f"📝 Processing case {i+1}: {case}")
                    
                    case_data = {
                        "rank": i + 1,
                        "case_number": str(case.get("case_id", case.get("case_number", f"case_{i+1}"))),
                        "category": str(case.get("category", "unknown")),
                        "similarity_score": Decimal(str(float(case.get("similarity", case.get("confidence_score", 0))))),
                        "reasoning": str(case.get("summary", case.get("reasoning_details", "")))[:500],
                        "basic_fault_ratio": str(case.get("base_fault_ratio", case.get("fault_ratio", "unknown"))),
                        "key_modifiers": case.get("modifications", case.get("applicable_modifications", []))[:10],
                        "source_file": str(case.get("source_file", "unknown"))
                    }
                    similar_data.append(case_data)
                    logger.info(f"✅ Processed case {i+1}: {case_data['case_number']}")
                    
                except Exception as case_error:
                    logger.error(f"❗ Error processing case {i+1}: {case_error}")
                    logger.error(f"❗ Case data: {case}")
                    continue
            
            logger.info(f"📊 Successfully processed {len(similar_data)} cases for DynamoDB")

        # Try to get existing item
        try:
            logger.info(f"🔍 Checking for existing item in DynamoDB...")
            response = table.get_item(Key={"connection_id": connection_id})
            item = response.get("Item")
            logger.info(f"📋 Existing item: {'Found' if item else 'Not found'}")
        except Exception as get_error:
            logger.warning(f"⚠️ Could not get existing item: {get_error}")
            item = None

        # Store to DynamoDB
        if item:
            logger.info(f"🔄 Updating existing DynamoDB record...")
            update_expr = "SET #ts = :ts, lat = :lat, lon = :lon, updated_at = :updated"
            expr_attr_names = {"#ts": "timestamp"}
            expr_attr_values = {
                ":ts": final_datetime,
            ...
            if similar_data:
                ...
                expr_attr_values[":similar"] = similar_data
                expr_attr_values[":complete"] = True
                logger.info(f"📊 Adding {len(similar_data)} similar cases to update")
            ...
            logger.info(f"✅ Successfully updated DynamoDB record")
            
        else:
            logger.info(f"➕ Creating new DynamoDB record...")
            new_item = {
                "connection_id": connection_id,
                "timestamp": final_datetime,
                "lat": Decimal(str(location["lat"])),
            ...
            if similar_data:
                new_item["similar_cases"] = similar_data
                new_item["case_analysis_complete"] = True
                logger.info(f"📊 Adding {len(similar_data)} similar cases to new item")

            table.put_item(Item=new_item)
            logger.info(f"✅ Successfully created new DynamoDB record")
        
        logger.info(f"🎉 DynamoDB storage completed successfully for {connection_id}")
            
    except Exception as e:
        logger.error(f"❗ CRITICAL: DynamoDB storage failed for {connection_id}")
        ...
        logger.error(f"❗ Failed data - connection_id: {connection_id}")
        logger.error(f"❗ Failed data - datetime: {datetime_str}")
        logger.error(f"❗ Failed data - location: {location}")
        logger.error(f"❗ Failed data - similar_cases: {similar_cases}")
