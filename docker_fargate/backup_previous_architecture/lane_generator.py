...
class IntegratedRoadNetworkGenerator:
    def __init__(self, connection_id):
        ...
        # Define paths based on pipeline
        self.mask_path = "final_road_mask_cleaned.png"
        self.roadmap_path = f"{connection_id}_cleaned_roadmap.png"
        self.satellite_path = "satellite.png"
        
        # Output files with connection_id prefix
        self.output_integrated_json = f"{connection_id}_integrated_road_network.json"
        self.output_centerlines_json = f"{connection_id}_centerlines_with_metadata.json"
        self.output_intersections_json = f"{connection_id}_intersections_with_metadata.json"
        self.output_lane_trees_json = f"{connection_id}_lane_tree_routes_enhanced.json"
        self.output_img = f"{connection_id}_integrated_network_visualization.png"
        self.output_metadata_only_json = f"{connection_id}_metadata_only_network.json"
        self.output_roads_metadata_json = f"{connection_id}_roads_metadata_only.json"
        self.output_intersections_metadata_json = f"{connection_id}_intersections_metadata_only.json"
        self.output_lanes_metadata_json = f"{connection_id}_lanes_metadata_only.json"
        
        # Debug files with connection_id prefix
        self.debug_skeleton = f"{connection_id}_debug_skeleton.png"
        self.debug_cleaned = f"{connection_id}_debug_cleaned_mask.png"
        self.debug_simplified = f"{connection_id}_debug_simplified_skeleton.png"
        self.debug_obstacles_analysis = f"{connection_id}_debug_obstacles_analysis.png"
        self.debug_mask_before_clean = f"{connection_id}_debug_mask_before_obstacle_removal.png" 
        self.debug_mask_after_clean = f"{connection_id}_debug_mask_after_obstacle_removal.png"
        self.debug_fragments_analysis = f"{connection_id}_debug_fragments_analysis.png"
        self.debug_skeleton_cleaned = f"{connection_id}_debug_skeleton_after_fragment_removal.png"
    ...
    def save_metadata_only_outputs(self):
        ...
        with open(self.output_roads_metadata_json, 'w') as f:
        ...
        with open(self.output_intersections_metadata_json, 'w') as f:
        ...
        with open(self.output_lanes_metadata_json, 'w') as f:
        ...
        with open(self.output_metadata_only_json, 'w') as f:
            json.dump(comprehensive_metadata_only, f, indent=2)
        
        print(f"  💾 Saved comprehensive metadata-only to {self.output_metadata_only_json}")
        print(f"  💾 Saved roads metadata-only to {self.output_roads_metadata_json}")
        print(f"  💾 Saved intersections metadata-only to {self.output_intersections_metadata_json}")
        print(f"  💾 Saved lanes metadata-only to {self.output_lanes_metadata_json}")
    ...
    def find_obstacles_in_roads(self, mask_path):
        ...
        cv2.imwrite(self.debug_mask_before_clean, mask)
    ...
    def remove_ignorable_obstacles(self, mask_path, obstacles, ignorable_obstacle_ids):
        ...
        cv2.imwrite(self.debug_mask_after_clean, cleaned_mask)
        return cleaned_mask
    ...
    def smart_obstacle_removal_pipeline(self, mask_path):
        ...
        classification = self.analyze_obstacles_with_bedrock(obstacles, mask_path, self.roadmap_path, self.satellite_path)
    ...
    def extract_medial_axis(self, cleaned_mask_image):
        ...
        cv2.imwrite(self.debug_skeleton, skel * 255)
    ...
    def find_skeleton_fragments(self, skeleton):
        ...
        cv2.imwrite(self.debug_fragments_analysis, debug_img)
        print(f"  💾 Saved fragment analysis to {self.debug_fragments_analysis}")
    ...
    def analyze_fragments_with_bedrock(self, fragments, expected_road_count=None):
        ...
        mask_b64 = self.encode_image_to_base64(self.debug_fragments_analysis)
    ...
    def remove_ignorable_fragments(self, skeleton, fragments, ignorable_fragment_ids):
        ...
        cv2.imwrite(self.debug_skeleton_cleaned, cleaned_skeleton * 255)
        print(f"  💾 Saved cleaned skeleton to {self.debug_skeleton_cleaned}")
    ...
    def debug_lane_display_names(self):
        """Diagnostic: Track exactly where ??? appears and verify consistency"""
        print("\n🔍 DIAGNOSTIC: TRACKING NAMING CONSISTENCY...")
        
        total_lanes_checked = min(5, len(self.lane_trees))  # Check first 5 or all if fewer
        issues_found = 0
        consistent_names = 0
        
        for i, lane_tree in enumerate(self.lane_trees[:total_lanes_checked]):
            lane_id = lane_tree.get('lane_id', 'NO_ID')
            road_id = lane_tree.get('road_id', 'NO_ROAD_ID')
            direction = lane_tree.get('direction', 'NO_DIRECTION')
            
            metadata = lane_tree.get('metadata', {})
            display_name = metadata.get('display_name', 'NO_DISPLAY_NAME')
            name = metadata.get('name', 'NO_NAME')
            
            # Check parent road for comparison
            source_road = next((r for r in self.roads if r['id'] == road_id), None)
            if source_road:
                road_metadata = source_road.get('metadata', {})
                road_display_name = road_metadata.get('display_name', 'NO_ROAD_DISPLAY_NAME')
                
                expected_lane_display_name = f"{road_display_name} ({direction} Lane)"
                expected_lane_name = f"{direction} Lane of {road_display_name}"
                
                display_name_matches = display_name == expected_lane_display_name
                name_matches = name == expected_lane_name
                has_question_marks_lane = '???' in str(display_name) or '???' in str(name)
                has_question_marks_road = '???' in str(road_display_name)
                
                print(f"🔍 Lane {i} ({lane_id}):")
                print(f"    Lane display_name: '{display_name}'")
                print(f"    Lane name: '{name}'")
                print(f"    Parent road display_name: '{road_display_name}'")
                print(f"    Expected lane display_name: '{expected_lane_display_name}'")
                print(f"    Expected lane name: '{expected_lane_name}'")
                print(f"    Display name matches: {display_name_matches}")
                print(f"    Name matches: {name_matches}")
                print(f"    Has ??? in lane: {has_question_marks_lane}")
                print(f"    Has ??? in road: {has_question_marks_road}")
                
                if display_name_matches and name_matches and not has_question_marks_lane:
                    print(f"    ✅ CONSISTENT AND CLEAN")
                    consistent_names += 1
                else:
                    print(f"    ❌ INCONSISTENT OR HAS ISSUES")
                    issues_found += 1
                    
                print(f"    ----")
            else:
                print(f"🔍 Lane {i} ({lane_id}): ❌ NO PARENT ROAD FOUND")
                issues_found += 1
        
        print(f"\n📊 NAMING CONSISTENCY SUMMARY:")
        print(f"  ✅ Consistent lanes: {consistent_names}/{total_lanes_checked}")
        print(f"  ❌ Issues found: {issues_found}/{total_lanes_checked}")
        print(f"  🎯 Consistency rate: {(consistent_names/total_lanes_checked)*100:.1f}%" if total_lanes_checked > 0 else "0%")
        
        if issues_found > 0:
            print(f"  ⚠️  WARNING: {issues_found} lanes have naming issues that may cause JSON/PNG inconsistency")
        else:
            print(f"  🎉 SUCCESS: All checked lanes have consistent naming")
    ...
    def save_lane_trees(self):
        ...
        with open(self.output_lane_trees_json, 'w') as f:
            json.dump(output_data, f, indent=2)
        
        print(f"  💾 Saved lane trees to {self.output_lane_trees_json}")
    ...
    def save_outputs(self):
        ...
        with open(self.output_integrated_json, 'w') as f:
        ...
        with open(self.output_centerlines_json, 'w') as f:
        ...
        with open(self.output_intersections_json, 'w') as f:
            json.dump(intersections_data, f, indent=2)
        
        print(f"💾 Saved integrated data to {self.output_integrated_json}")
        print(f"💾 Saved separate centerlines to {self.output_centerlines_json}")
        print(f"💾 Saved separate intersections to {self.output_intersections_json}")
    ...
    def visualize_integrated_network_with_comprehensive_metadata(self):
        ...
        # Save visualization
        cv2.imwrite(self.output_img, canvas)
        cv2.imwrite("comprehensive_narrative_network_visualization.png", canvas)
        print(f"  💾 Saved network visualization to {self.output_img}")

    def visualize_integrated_lanes_with_comprehensive_metadata(self):
        """COMPREHENSIVE lane visualization with ALL narrative metadata"""
        ...
        cv2.imwrite("comprehensive_narrative_lane_visualization.png", canvas)
        print(f"  💾 Saved COMPREHENSIVE narrative lane visualization")

    def debug_metadata_flow(self):
        """Enhanced debug to check lane names and directions"""
        print("\n🔍 DEBUGGING LANE NAMES AND DIRECTIONS...")
        
        print("📊 LANE NAME AND DIRECTION CHECK:")
        for i, lane in enumerate(self.lane_trees[:4]):  # Check first 4 lanes
            metadata = lane.get('metadata', {})
            lane_id = lane.get('lane_id', 'NO_ID')
            direction = lane.get('direction', 'NO_DIRECTION')
            display_name = metadata.get('display_name', 'NO_NAME')
            geographic_direction = metadata.get('geographic_direction', 'NO_GEO')
            original_direction = metadata.get('original_direction', 'NO_ORIG')
            
            print(f"  Lane {i}:")
            print(f"    Lane ID: {lane_id}")
            print(f"    Direction: {direction}")
            print(f"    Display Name: '{display_name}'")
            print(f"    Geographic Direction: {geographic_direction}")
            print(f"    Original Direction: {original_direction}")
            print(f"    Has ??? in name: {'???' in display_name}")
            
            # Check if direction makes sense
            valid_directions = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"]
            direction_valid = direction in valid_directions
            print(f"    Direction Valid: {direction_valid}")
            print()

    def verify_json_data_integrity(self):
        """Verify that JSON data has proper intersection zones and lane connections"""
        print("\n🔍 VERIFYING JSON DATA INTEGRITY...")
        
        # Check intersection zones
        print("📍 INTERSECTION ZONES:")
        for intersection in self.intersections:
            int_id = intersection['id']
            center = intersection['center']
            zone = intersection.get('zone', {})
            ...
            if zone:
                ...
                boundary_points = len(zone.get('boundary_points', []))
                connection_points = len(zone.get('connection_points', []))
                
                print(f"  ✅ Intersection {int_id}: center={center}, radius={radius}")
                print(f"      📍 {boundary_points} boundary points, {connection_points} connection points")
                
                # Check metadata
                intersection_name = metadata.get('intersection_name', 'N/A')
                traffic_volume = metadata.get('traffic_volume', 'N/A')
                print(f"      🏷️  Name: {intersection_name}, Traffic: {traffic_volume}")
            else:
                print(f"  ❌ Intersection {int_id}: NO ZONE DATA")
        
        # Check road connections
        print("\n🔗 ROAD CONNECTIONS:")
        road_connections = getattr(self, 'road_connections', {})
        for int_id, connections in road_connections.items():
            print(f"  📍 Intersection {int_id}: {len(connections)} connected roads")
            for conn in connections:
                road_id = conn.get('road_id', '?')
                conn_type = conn.get('connection_type', '?')
                distance = conn.get('closest_distance', 0)
                road_name = conn.get('road_name', 'N/A')
                print(f"      ✅ Road {road_id} ({road_name}): {conn_type}, distance={distance:.1f}")
        
        # Check lane tree branches
        print("\n🛤️  LANE TREE CONNECTIONS:")
        total_branches = 0
        connected_lanes = 0
        lanes_with_metadata = 0
        
        for tree in self.lane_trees:
            lane_id = tree.get('lane_id', '?')
            branches = tree.get('branches', [])
            road_id = tree.get('road_id', '?')
            metadata = tree.get('metadata', {})
            
            if metadata:
                lanes_with_metadata += 1
                road_name = metadata.get('display_name', 'N/A')
                road_class = metadata.get('road_class', 'N/A')
            else:
                road_name = 'N/A'
                road_class = 'N/A'
            ...
            if branches:
                ...
                print(f"  ✅ Lane {lane_id} ({road_name}, {road_class}): {len(branches)} branches")
                
                # Show first branch as example
                if branches:
                    first_branch = branches[0]
                    target_name = first_branch.get('target_road_name', 'N/A')
                    turn_type = first_branch.get('turn_type', 'N/A')
                    print(f"      🌟 Example branch: {turn_type} to {target_name}")
            else:
                print(f"  ⚠️  Lane {lane_id} ({road_name}, {road_class}): NO BRANCHES")
        
        print(f"\n📊 INTEGRATED DATA QUALITY SUMMARY:")
        print(f"  🚦 Intersections with zones: {sum(1 for i in self.intersections if 'zone' in i)}/{len(self.intersections)}")
        print(f"  🏷️  Intersections with metadata: {sum(1 for i in self.intersections if 'metadata' in i)}/{len(self.intersections)}")
        print(f"  🔗 Roads with connections: {sum(len(conns) for conns in road_connections.values())}")
        print(f"  🛣️  Roads with metadata: {sum(1 for r in self.roads if 'metadata' in r)}/{len(self.roads)}")
        print(f"  🛤️  Lanes with branches: {connected_lanes}/{len(self.lane_trees)}")
        print(f"  🏷️  Lanes with metadata: {lanes_with_metadata}/{len(self.lane_trees)}")
    ...
    def process_integrated_network_with_comprehensive_metadata_and_clean_trimming(self):
        ...
        try:
            ...
            if not os.path.exists(self.roadmap_path):
                print(f"❌ Roadmap file not found: {self.roadmap_path}")
                # Try fallback
                if os.path.exists("roadmap.png"):
                    self.roadmap_path = "roadmap.png"
                    print(f"  📍 Using fallback: {self.roadmap_path}")
                else:
                    print(f"❌ No roadmap file available")
                    sys.exit(1)
            
            if not os.path.exists(self.satellite_path):
                print(f"❌ Satellite file not found: {self.satellite_path}")
                sys.exit(1)

            # STEP 0: Clean obstacle removal
            print("\n🎯 STEP 0: LLM obstacle removal...")
            cleaned_mask = self.smart_obstacle_removal_pipeline(self.mask_path)
            
            # STEP 0.5: LLM network structure analysis
            print("\n🔢 STEP 0.5: LLM network structure analysis...")
            network_analysis = self.analyze_expected_road_and_intersection_count_with_bedrock(
                self.debug_mask_after_clean, self.roadmap_path, self.satellite_path)
            expected_roads = network_analysis.get('expected_main_roads', 4)
            expected_intersections = network_analysis.get('expected_major_intersections', 1)
            
            # STEP 1: Clean skeleton processing
            ...
            # STEP 3.5: CRITICAL - Road trimming at intersections
            print(f"\n✂️  STEP 3.5: Trimming roads at intersection boundaries...")
            self.trim_roads_at_intersection_zones()

            # STEP 4: COMPREHENSIVE METADATA EXTRACTION 
            print("\n🏷️  STEP 4: Extracting comprehensive metadata...")
            self.comprehensive_metadata = self.extract_comprehensive_metadata_with_bedrock(self.roadmap_path, self.satellite_path)
            
            # STEP 5: COMPREHENSIVE METADATA ASSIGNMENT
            print("\n🎯 STEP 5: Assigning comprehensive metadata to trimmed roads...")  
            self.assign_metadata_to_roads()
            
            print("\n🚦 STEP 6: Assigning comprehensive metadata to intersections...")
            self.assign_metadata_to_intersections()

            # STEP 7: NARRATIVE METADATA GENERATION 
            print("\n💬 STEP 7: Generating narrative metadata for route matching...")
            self.narrative_metadata = self.generate_narrative_metadata_for_route_matching(self.roadmap_path, self.satellite_path)
            
            # STEP 8: NARRATIVE METADATA ASSIGNMENT
            print("\n🎯 STEP 8: Assigning narrative metadata to roads...")  
            self.assign_narrative_metadata_to_roads()
            
            print("\n🚦 STEP 9: Assigning narrative metadata to intersections...")
            self.assign_narrative_metadata_to_intersections()
            
            # STEP 10: CLEAN road connections 
            print("\n🔗 STEP 10: Building road-intersection zone connections...")
            self.build_road_connections_for_zones()
            
            # STEP 11: Save comprehensive outputs (roads + intersections with metadata)
            print("\n💾 STEP 11: Saving comprehensive outputs...")
            self.save_outputs()
            
            # STEP 12: INTEGRATED visualization with comprehensive metadata
            print("\n🎨 STEP 12: Creating integrated network visualization...")
            self.visualize_integrated_network_with_comprehensive_metadata()
                        
            print("\n🛤️  STEP 13: Generating traffic-flow based lanes...")
            self.generate_geographic_lanes_from_roads()

            print("\n🚗 STEP 14: Enriching lanes with traffic geometry for LHT...")
            self.enrich_lanes_with_traffic_geometry()

            print("\n💬 STEP 15: Adding narrative to traffic mapping...")
            self.add_narrative_to_traffic_mapping()

            print("\n🚗 STEP 16: Adding LHT lane usage guidance...")
            self.add_lht_lane_usage_guidance()

            print("\n🔄 STEP 17: Ensuring metadata consistency...")
            self.ensure_metadata_consistency()

            print("\n🔍 STEP 18: Debugging metadata flow...")
            self.debug_metadata_flow()

            print("\n💾 STEP 19: Saving comprehensive lane trees...")
            self.save_lane_trees()

            print("\n🔍 STEP 20: Final check - Verifying lane display names...")
            self.debug_lane_display_names()

            print("\n🎨 STEP 21: Creating integrated lane visualization...")
            self.visualize_integrated_lanes_with_comprehensive_metadata()
            
            # STEP 22: Comprehensive data integrity verification
            print("\n🔍 STEP 22: Verifying integrated data integrity...")
            verification_report = self.verify_json_data_integrity()

            # STEP 23: Save metadata-only JSON files for LLM
            print("\n📄 STEP 23: Saving metadata-only JSON files for LLM consumption...")
            ...
            print(f"💥 Error during road network generation: {e}")
            import traceback
            traceback.print_exc()
            sys.exit(1)
...
def main():
    ...
    print(f"🚗 ROAD NETWORK GENERATOR for connection_id: {connection_id}")
    print(f"📍 Processing road network generation...")
